...
lib/rcas/van_hoeij.rb       Factor::VanHoeij: recombination by LLL [vHo02] on the coefficients of f*g'/g [HvHN11], bounded through Fujiwara's root bound [Fuj16]. Rounds of two columns each (smallest bound first); after LLL, cut at the last |b*_s|**2 <= r + columns; stop when the rref of the 0/1 part is a partition and every candidate divides; else double the precision (MAX_RAISES) and then fall back to the subsets. `Factor.recombination` (thread-local, `with_recombination`, nil inherits) is :auto/:van_hoeij/:zassenhaus. :auto runs the subsets while C(r, next size) <= SUBSET_BUDGET = 50_000 and then hands the cofactor *and its lifted factors* to the lattice (`recombine(..., lifted:)`; a second Hensel lift from p was 0.8 of 3.7 s on SD(6)). A threshold on the number r of local factors was measured and rejected (24 Sept 2026): products of small factors are twice as fast by subsets at every r from 17 to 26, because the true factors turn up among pairs and triples; what makes the subsets explode is the size of the subsets the true factors need. Budgets of 5_000-10_000 cost that mix 60%. Products of *translated* SD polynomials are the hard shape for subsets (each true factor needs 8 or 16 local factors): SD(5)*SD(5)(x+1) 2.9 s by :auto, > 120 s by subsets; three translated SD(5)s (r = 48) 15 s; four SD(4)s 2.8 s against 11.5 s
...
lib/rcas/matrix_multiply.rb MatrixMultiply: Matrix#* and #multiply(algorithm:). Integer/Rational entries on the bare numbers (rows and columns scaled by their denominators, one division per entry at the end: 15x faster and 1% of the objects of the Scalar route, which built a Num per product); Strassen in Winograd's form [Str69], [Win71] on blocks (`IntegerBlocks`/`ScalarBlocks` do the block arithmetic, one recursion for both; odd sizes padded for one level). `MatrixMultiply.algorithm` is thread-local like Factor.recombination (:auto/:strassen/:schoolbook, nil inherits). :auto was measured (24 Sept 2026): Strassen from n = 192 for machine-word entries (leaves of 48), from 64 past LARGE_BITS (leaves of 16), never for symbolic entries (the products of sums expand again); `test_strassen_multiplies_two_by_two_matrices_with_seven_products` counts Scalar.mul calls
lib/rcas/multimodular.rb    Multimodular: det/inverse/solve of Integer/Rational matrices modulo primes just below 2**31 (residue products stay Fixnums), CRT by Garner, the number of primes from Hadamard's bound [Had93] (min of row and column products) - so the answer is a proof, never early termination on agreement; det(A)*A**-1*B through Cramer's rule, whose numerators are bounded by |b| times the other column lengths; a prime dividing det A counts for det and is skipped for the adjugate (`test_primes_that_divide_the_determinant_are_skipped_for_the_inverse` fails if it is counted). Rational rows scaled to Integers first (right-hand sides with them). Matrix#det/inverse/solve take `algorithm:` (:auto/:multimodular/:elimination); :auto takes the primes for every Integer/Rational matrix, since they won at every size measured (2x2: 20 vs 23 us; n = 100: det 1.09 -> 0.24 s, inverse 5.9 -> 1.1 s). A singular square system goes back to rref (free variables 0). Solve and inverse also stop *early* (`crt_solve(early: true)`): after 1, 2, 4, ... usable primes the residues of A**-1*B are rationally reconstructed with one common denominator (`reconstruct`, grown entry by entry) and accepted only if A*N = d*B exactly (`verified`) - without that check a too-small modulus gives a wrong fraction (10**12/3 mod one prime reads back as 8918/17289; dixon_test). No overhead on random matrices (the bound is nearly tight there), 5x on the inverse of a determinant-1 matrix. det has no check and always takes the bound
lib/rcas/dixon.rb           Dixon: one square system by p-adic lifting [Dix82]: A**-1 mod one prime (the first not dividing det A; a Hadamard-sized product of primes all dividing it proves singular), then x_i = C*r mod p, r = (r - A*x_i)/p, reconstruction tried at steps 4, 8, 16, ... through Multimodular.verified; `2*max(N, D)**2` is where it must succeed. Multimodular.solver picks it for :auto from n = 32 (DIXON_MIN, measured: level at 24-30 for any entry size, 3-6x at 100-150); inverse keeps the primes (lifting n columns costs n**3 per digit)
...
- **Run the suite under Ruby 4 too** before a review round:
  ...
  (Homebrew's; its default parser is Prism and its Hash#inspect is the 3.4 one). Both were green on 24 Sept 2026: 1262 runs, 5 skips under Ruby 4 (the chat tests without the anthropic gem).
