...
module RCAS
  # QQ**3: column vectors of fixed dimension over a domain. Over a ring such
  # as ZZ this is a free module rather than a vector space, but the API is
  # the same.
  class VectorSpace
    attr_reader :domain, :dim

    def initialize(domain, dim)
    ...
    def [](*entries)
      ...
      entries = entries.map { |e| domain.normalize_coefficient(e) } if domain.respond_to?(:normalize_coefficient)
      entries.each do |e|
        next if domain.include?(e)
        hint = e.variables.reject { |v| RCAS.assumption(v) }
        hint = hint.empty? ? "" : " (declare #{hint.join(', ')} with assume(#{hint.first}: #{domain}))"
        raise DomainError, "#{e} is not in #{domain}#{hint}"
      end
    ...
    def include?(obj)
      ...
      obj.is_a?(Vector) && obj.dim == dim && obj.entries.all? { |e| domain.include?(e) }
    rescue DomainError
      false
    end
    ...
    def over(other_domain) = VectorSpace.new(other_domain, dim)

    def ==(other) = other.is_a?(VectorSpace) && other.domain == domain && other.dim == dim
    alias eql? ==
    def hash = [VectorSpace, domain, dim].hash
  ...
  class Vector
    ...
    def cross(other)
      ...
      VectorSpace.new(domain.join(other.domain), 3).unchecked([
    ...
    def zip_with(other, op)
      ...
      check_dim!(other)
      VectorSpace.new(domain.join(other.domain), dim).unchecked(entries.zip(other.entries).map { |a, b| yield a, b })
    end
