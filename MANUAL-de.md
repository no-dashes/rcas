...
- [1. Mathematik](#1-mathematik)
  ...
  - [1.3 Analysis](#13-analysis)
    ...
    - [Kurven- und Flächenintegrale](#kurven--und-flächenintegrale)
    - [Green, Stokes und Gauß](#green-stokes-und-gauß)
    - [Reihen](#reihen)
...
- [Anhang B. rcas-chat](#anhang-b-rcas-chat)
  ...
  - [Dateien](#dateien)
- [Anhang C. rcas-app](#anhang-c-rcas-app)
  - [Wie es gebaut ist](#wie-es-gebaut-ist)
  - [Starten](#starten)
  - [Eingabe](#eingabe)
  - [Ausgabe](#ausgabe)
  - [Befehle](#befehle)
  - [Im Dock, im Startmenü, in der Programmliste](#im-dock-im-startmenü-in-der-programmliste)
  - [Wer die Sitzung erreicht, und wer nicht](#wer-die-sitzung-erreicht-und-wer-nicht)
  - [Optionen und Umgebung](#optionen-und-umgebung)
  - [Dateien](#dateien)
...
`[x(t), y(t)]`, im Raum mit drei Komponenten; `revolution_volume` und `revolution_surface` drehen einen
...
Ein Kurvenintegral summiert eine Funktion längs einer Kurve auf, ein
Flächenintegral über eine Fläche. Beide werden zu gewöhnlichen Integralen,
sobald die Kurve oder die Fläche parametrisiert ist: eine Kurve
`[x(t), y(t)]` (im Raum mit drei Komponenten) trägt das Längenelement
`ds = |r'(t)|dt`, eine Fläche `[x(u, v), y(u, v), z(u, v)]` das
Flächenelement `dS = |r_u x r_v|du dv`. `line_integral(f, curve, t: a..b)`
integriert ein Skalarfeld gegen `ds`; mit einem *Vektorfeld* integriert es
gegen `dr`, und das ist die Arbeit, die das Feld längs der Kurve
verrichtet. `flux` stellt dieselbe Frage an die Normale: über eine ebene
Kurve integriert es `F.n ds`, nach außen, wenn die Kurve im Gegenuhrzeiger-
sinn durchlaufen wird. Das Feld wird in `x`, `y`, `z` gelesen, sofern nicht
`vars: [u, v]` andere Koordinaten nennt.
...
Das letzte ist der Satz von Green, rückwärts gelesen: der Flächeninhalt
innerhalb einer geschlossenen Kurve ist ein Kurvenintegral um sie herum,
hier um eine Astroide.

Über eine Fläche wird in der Reihenfolge integriert, in der die beiden
Parameter genannt werden, der erste innen, wie bei `integrate`. Diese
Reihenfolge orientiert die Fläche auch, denn die Normale ist `r_u x r_v`
mit dem ersten Parameter `u` und dem zweiten `v`: vertauscht man die beiden
Bereiche, dreht sich die Normale um, und der Fluss wechselt mit ihr das
Vorzeichen. Neue Integrale sind das alles nicht, und deshalb bleibt stehen,
was `integrate` nicht kann: ein `integral(...)`-Knoten, den `evalf` zu Ende
rechnet.
...
An der Kugel sieht man, worauf es beim Längenelement ankommt:
`|r_u x r_v|` ist `sqrt(a**4*sin(v)**2)`, und die Wurzel aus einem Quadrat
ist der Betrag. rcas zieht den Faktor mit dem Vorzeichen aus der Wurzel,
das er auf dem Parameterbereich hat - `sin(v)` ist auf `0..pi` positiv -
und schreibt `abs(...)`, wo das Vorzeichen dort wechselt, statt
stillschweigend eines anzunehmen.

#### Green, Stokes und Gauß

Die drei Sätze der Vektoranalysis sagen alle dasselbe: ein Integral über
einen Rand ist gleich einem Integral einer Ableitung über das, was er
berandet. Der Satz von Green macht aus der Zirkulation von `[P, Q]` um den
Rand eines ebenen Bereichs das Doppelintegral von `Q_x - P_y` über den
Bereich; der Satz von Stokes aus der Zirkulation um den Rand einer Fläche
den Fluss der Rotation durch sie; der Satz von Gauß aus dem Fluss durch den
Rand eines Körpers das Dreifachintegral der Divergenz über ihn. Jede
Funktion rechnet die Seite über dem Bereich aus, meist die einfachere; die
Integrale des vorigen Abschnitts rechnen die andere, so daß man beide
...
Die Bereiche beschreiben das Gebiet, der innerste zuerst, und deshalb
dürfen die inneren Grenzen von der äußeren Variablen abhängen: die zweite
Zeile integriert über ein Dreieck, `0 <= y <= x <= 1`. Die dritte und die
vierte sind die beiden Seiten des Satzes von Stokes für die Einheitskreis-
scheibe, unabhängig voneinander gerechnet.

Ein Feld, dessen Zirkulation um jede geschlossene Kurve verschwindet, ist
der Gradient eines Potentials, und dann hängt die Arbeit nur von den Enden
des Weges ab. `conservative?` prüft das an der Symmetrie der Ableitungen,
und `potential` integriert das Feld Komponente für Komponente zurück; ein
Feld ohne Potential sagt das, statt etwas zurückzugeben, das keines ist.
...
| Kurven- und Flächenintegrale | `line_integral surface_integral flux enclosed_area green stokes divergence_theorem conservative? potential` |
...
dreidimensionale Grafiken, Geometrie im Raum, Flächen, die implizit statt
durch eine Parametrisierung gegeben sind,
...
Koeffizienten nicht hypergeometrisch sind (`tan`, `exp(x)/(1 - x)`) sowie von
OpenMath die binäre Kodierung und Content-MathML (Anhang D).
...
| Kurven- und Flächenintegrale, Green, Stokes und der Satz von Gauß | vector_calculus.rb | [MT12, ch. 7-8]; [Spi65, ch. 4-5] |
...
- [OM19] S. Buswell, O. Caprotti, D. P. Carlisle, M. C. Dewar, M.
  Gaetano, M. Kohlhase (Hrsg.), *The OpenMath Standard, version 2.0*,
  Fassung vom 2019-07-01, The OpenMath Society, zusammen mit den
  offiziellen Content Dictionaries auf openmath.org/cd.
...
Gleichungen und Ableitungen, eine Nullstelle ohne Wurzelform, ebenso
Symbole, Zahlen, Arrays und Hashes. Das
...
Satz. Sie tippen Ruby und bekommen das Ergebnis in iTerm2 gesetzt und
sonst als Text; `/output both` zeigt zusätzlich den Text.
...
| `typeset` | nur das gesetzte Bild, Text, wenn ein Wert keine LaTeX-Form hat (die Voreinstellung in iTerm2) |
| `both` | Text, dann das Bild |
...
`bin/rcas-app` ist die dritte Bedienoberfläche: ein Fenster. Es zeigt die
Sitzung als Arbeitsblatt aus nummerierten `In`/`Out`-Zellen, setzt jedes
Ergebnis, das eine LaTeX-Form hat, typografisch und zeichnet Grafiken als
Bilder statt als Braille-Kunst.
...
Die gesetzten Zeilen sind KaTeX, also echter mathematischer Satz und nicht
die Näherung, die diese ASCII-Skizze zeigen kann.

### Wie es gebaut ist

Das Fenster ist ein Browser der Chromium-Familie, gestartet mit
`--app=URL`. Dieser Schalter zeichnet ein einzelnes Fenster ohne Tabs,
ohne Adresszeile und ohne Lesezeichen: die Browser-Maschine ohne den
Browser darum herum. Den Schalter gibt es unter macOS, Windows und Linux,
ein Starter genügt also für alle drei.

Die Maschine ist *geliehen, nicht mitgeliefert*. Eine Anwendung, wie man
sie üblicherweise baut, mit Electron, trägt ihre eigene Kopie von Chromium
und wiegt mehrere hundert Megabyte; dies hier sind die Bibliothek, eine
HTML-Seite und vier kleine Ruby-Dateien. Der Preis ist, dass ein solcher
Browser installiert sein muss - was rcas ohnehin verlangt, wenn es mit dem
`:katex`-Backend setzt (Anhang A). Chrome, Chromium, Brave und Microsoft
Edge gehen alle; `RCAS_BROWSER` benennt einen anderen.

Alles Mathematische geschieht in Ruby, in demselben Sitzungsobjekt, das
auch `bin/rcas` und `bin/rcas-chat` benutzen (`Chat::Workspace`). Bloße
Namen sind also Unbestimmte, und die Nummerierung von `In` und `Out`
bedeutet dasselbe wie überall sonst. Die Seite schickt eine Eingabezeile
und zeichnet die Antwort; sie rechnet nichts selbst.
...
Das Programm endet, wenn das Fenster geschlossen wird. `--no-window`
startet nur den Server und gibt eine Adresse aus, die man von Hand öffnet:
...
Enter wertet die Zeile aus, Shift-Enter beginnt eine neue. Eine Zeile, die
noch nicht fertig ist - ein offener Block, eine offene Zeichenkette oder
Klammer -, wird von Enter ebenfalls nicht abgeschickt: gefragt wird Rubys
eigener Parser, `def f(x)` wartet also auf sein `end`.

Tab vervollständigt das Wort unter dem Cursor aus den Variablen der
Sitzung, den Funktionen der obersten Ebene und den Zahlbereichen; teilen
sich mehrere Namen einen Anfang, wird der gemeinsame Teil eingesetzt und
die Kandidaten werden unter der Eingabe angeboten. Pfeil hoch und runter
gehen durch die bereits getippten Zeilen, und ein Klick auf eine
`In`-Zeile im Arbeitsblatt holt sie zurück in die Eingabe.

### Ausgabe

Die vier Modi von `rcas-chat` stehen in der Titelzeile, und `/output`
stellt sie über die Tastatur ein:

| Modus | was ein Ergebnis zeigt |
|---|---|
| `text` | nur die rcas-Textform; Grafiken sind Braille-Kunst |
| `typeset` | nur die gesetzte Form (die Voreinstellung) |
| `both` | Text und gesetzte Form |
| `latex` | die Textform und ihren LaTeX-Quelltext |

`typeset` ist der Name, den das Fenster dem Modus gibt, den `rcas-chat`
und `~/.rcas/settings.json` `tex` nennen; `/output` nimmt beide
Schreibweisen. Ein Wert ohne LaTeX-Form fällt in diesem Modus auf seinen
Text zurück, es wird also nie etwas verborgen.

Grafiken sind SVG-Bilder, von `Plot#to_svg` in den Farben des Fensters
gezeichnet. `/theme dark`, `/theme light` und `/theme auto` (die
Voreinstellung, die dem Schreibtisch folgt) entscheiden über diese Farben.

`/help NAME` zeigt, was `doc(name)` weiß - die Signatur, den
Kommentarblock, Mathematik und Methode hinter dem Namen, seine Quellen und
seine Wikipedia-Artikel, letztere als Links, die sich im gewöhnlichen
Browser öffnen.
...
| Befehl | Bedeutung |
|---|---|
| `/help [NAME]` | die Befehle, oder was eine Funktion, Menge oder Klasse tut |
| `/output [text\|typeset\|both\|latex]` | wie Ergebnisse gezeigt werden |
| `/theme [dark\|light\|auto]` | das Farbschema des Fensters |
| `/unicode [on\|off]` | ℤ, π und ∞ statt ZZ, pi und oo ausgeben |
| `/numbered [on\|off]` | die Zeilen der Sitzung nummerieren |
| `/latex EXPR` | der LaTeX-Quelltext eines Ruby-Ausdrucks |
| `/vars` | die Variablen der Sitzung |
| `/assumptions` | erklärte Bereiche von Unbestimmten |
| `/forget [x ...]` | Bereichsannahmen vergessen |
| `/save [FILE]` | ein Markdown-Protokoll speichern |
| `/reset` | von vorn: Variablen, Annahmen und Nummerierung |
| `/clear` | das Arbeitsblatt leeren, die Variablen behalten |
| `/exit` | das Fenster schließen |

Alles andere ist Ruby. Das Fenster spricht nicht mit Claude; Fragen in
natürlicher Sprache sind Sache von `rcas-chat` (Anhang B).

### Im Dock, im Startmenü, in der Programmliste
...
`--install` schreibt den Eintrag, den der Schreibtisch erwartet, und
`--uninstall` entfernt ihn wieder: ein Programmbündel in `~/Applications`
unter macOS, mit dem Logo, das macOS' eigene Werkzeuge `sips` und
`iconutil` in ein Symbol verwandeln; eine `.desktop`-Datei in
`~/.local/share/applications` unter Linux; eine Verknüpfung im Startmenü
unter Windows. Jeder Eintrag startet dieses Arbeitsverzeichnis mit diesem
Ruby, das Programm braucht also kein Terminal.

### Wer die Sitzung erreicht, und wer nicht

Fenster und Programm reden über HTTP auf der Loopback-Schnittstelle
miteinander. Das heißt, der Auswertungsendpunkt führt beliebiges Ruby aus
und muss deshalb für alles andere auf dem Rechner unerreichbar sein. Drei
Dinge halten ihn dort:

* Der Socket hängt an `127.0.0.1`, von außerhalb des Rechners kommt also
  überhaupt keine Verbindung zustande.
* Für jeden Lauf wird ein zufälliges Token erzeugt; es steht in der URL
  des Fensters und kommt in einem Kopffeld jedes Aufrufs zurück. Eine
  Seite anderer Herkunft kann es weder raten noch dieses Kopffeld setzen,
  weil der Server keinen CORS-Preflight beantwortet.
* Das `Host`-Kopffeld muss die Loopback-Adresse nennen. Das verhindert,
  dass ein öffentlicher Name, der auf `127.0.0.1` zeigt, die Sitzung
  erreicht (DNS-Rebinding).

Das Fenster hält sein eigenes Browser-Profil in `~/.rcas/app`; es ist also
ein anderer Prozess als das eigene Surfen und übernimmt nichts aus jener
Sitzung.
...
| Option | Bedeutung |
|---|---|
| `--port N` | auf diesem Port lauschen (Voreinstellung: irgendein freier) |
| `--output MODUS` | `text`, `typeset`, `both` oder `latex` |
| `--theme NAME` | `dark`, `light` oder `auto` |
| `--no-window` | nur den Server starten und die Adresse ausgeben |
| `--install`, `--uninstall` | den Schreibtisch-Eintrag anlegen oder entfernen |
| `--version`, `--help` | Version, Aufruf |

| Variable | Bedeutung |
|---|---|
| `RCAS_BROWSER` | der Browser, der das Fenster zeichnet |
| `RCAS_HOME` | wo Profil und Einstellungen liegen (Voreinstellung `~/.rcas`) |
| `RCAS_KATEX_DIR` | die KaTeX-Distribution, mit der gesetzt wird |

Ausgabemodus und Farbschema kommen auch aus `~/.rcas/settings.json`, der
Datei, die `rcas-chat` mit `/settings save` schreibt.
...
[OpenMath](https://openmath.org/) ist ein Standard für die *Bedeutung*
eines mathematischen Objekts, die Notation bleibt dabei außen vor. rcas
spricht es, ein Ausdruck kann die Sitzung also verlassen, anderswo
verstanden werden - und zurückkommen.

### Objekte, nicht XML

Der Kern des Standards wird leicht übersehen: ein OpenMath-Objekt ist ein
abstrakter Baum, und XML ist eine Art, ihn aufzuschreiben. rcas hält
beides auseinander. `openmath` baut das Objekt, und das Objekt druckt sich
als es selbst:
...
Content Dictionary ist ein kleines Dokument, das festhält, was ein Symbol
bedeutet, und es ist das, worüber zwei Systeme sich einig sind - nicht die
Schreibweise. `to_xml` ist dann eine Kodierung dieses Objekts:
...
`to_xml(indent: 2)` schreibt dasselbe Objekt mit einer Zeile je Element,
was sich leichter liest:
...
Ein Dokument, das 1 + 2 sagt, meint die Summe und nicht die Zahl, und
genau das gibt rcas zurück:
...
Ebenso ein Integral. Es kommt als das Integral an, das es ist, und `doit`
beantwortet es, wenn man danach fragt:
...
Eine gebundene Unbestimmte in rcas ist in OpenMath `fns1.lambda` - bei
Summen, Grenzwerten und Ableitungen ebenso wie bei Integralen:
...
Zahlen behalten ihre Art - eine rationale Zahl ist eine Anwendung von
`nums1.rational` und keine Dezimalzahl -, und Gleichungen, Ungleichungen,
Intervalle, Zahlbereiche, Matrizen, Vektoren, Listen und abschnittsweise
definierte Funktionen reisen ebenso mit wie Ausdrücke:
...
Zetafunktion, `erf`, die Integralfunktionen oder `RootOf`; diese stehen
deshalb im eigenen Content Dictionary von rcas, was das Symbol auch sagt:
...
Ein Symbol, von dem rcas noch nie gehört hat, ist weder ein Fehler noch
ein Anlass zum Raten. Es kommt als unbekannte Funktion zurück, benannt
nach ihrem Dictionary - dieselbe Art Knoten wie das `u(n + 1)` einer
Rekursion -, und es wird wieder genau zu dem Symbol kodiert, aus dem es
kam:
...
Wo die beiden Systeme wirklich auseinandergehen, sagt das Lesen es, statt
so zu tun als ob. `transc1.log` ist der Logarithmus zu einer Basis, wofür
rcas keinen Knoten hat; er kommt deshalb als der Quotient an, der er ist:
...
Der einseitige Grenzwert (`limit1.above`, `limit1.below`) ist der andere
Fall: der Grenzwertknoten von rcas trägt keine Richtung, ein solches
Objekt bleibt deshalb, wie es angekommen ist, statt seine Seite zu
verlieren, und geht unverändert wieder hinaus.

### Was fehlt

Die binäre Kodierung und striktes Content-MathML sind die beiden anderen
Arten, dieselben Objekte aufzuschreiben, und keine davon ist
implementiert; ansetzen würden sie am Objektmodell, nicht am Phrasebook.
Presentation-MathML ist etwas ganz anderes - Notation, der Nachbar von
Anhang A - und gehört zu `to_latex` und nicht hierher. Annahmen
